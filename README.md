...
<p align="center">
『学園アイドルマスター』のプロデューサー情報を Discord のアクティビティに表示できる<br>非公式ファンメイドアプリケーション
</p>
...
<p align="center">
  <a href="https://github.com/Wea017net/GkmStatus/releases/download/v1.1.0/GkmStatus_win-x64_v1.1.0.zip"><img src="https://raw.githubusercontent.com/Wea017net/.github/refs/heads/main/GkmStatus/resources/download_win_ja.png" height="54px" alt="download">
</p>
...
アクティビティにプロデューサーの情報を表示させることで、プロデューサー同士のコミュニケーションの活性化に繋がったり、ゲームのダウンロード誘導ボタンを表示させることで、新規プレイヤーの増加に繋がったりすることを期待します。
...
> [!NOTE]
> ARM 版 Windows、macOS、Linuxでの動作はサポート対象外です。

## エディションの違い

> [!TIP]
> GkmStatus では通常版とLite版の2つのエディションを用意しています。
> 環境に合わせてエディションを選択してください。

### 通常版 (`GkmStatus_win-x64_vx.x.x.zip`)

`GkmStatus.exe` 単体のみが含まれます。アプリ内に .NET Runtime 10.0 が同梱されているため、別途インストールすることなく即座に利用できます。

### Lite版 (`GkmStatus_win-x64_vx.x.x_lite.zip`)

`GkmStatus.exe` といくつかのファイルが含まれます。ファイルサイズが小さいですが、アプリ内に .NET Runtime 10.0 が同梱されていないため、あらかじめ PC にインストールしておく必要があります。
...
4.  ゲームの表示名や自分のプロデューサー情報など、Discord に表示させたい内容を入力します。
5.  接続ボタンを押すと、Discord のアクティビティに入力した内容が表示されます。

> [!NOTE]
> 「Windows によって PC が保護されました」というウィンドウが表示された場合、`詳細情報` をクリックしたあとにウィンドウ下部に表示される `実行` をクリックすることでアプリを起動できます。
